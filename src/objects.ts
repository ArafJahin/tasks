...
export function isValid(question: Question, answer: string): boolean {
    if (question.type === "short_answer_question") {
        ...
        return question.options.reduce(
            (totalAnswer: boolean, x: string) =>
                (totalAnswer = totalAnswer || answer === x),
            false
        );
    } else {
        return false;
...
export function toMarkdown(question: Question): string {
    const firstSecondLine = "# " + question.name + "\n" + question.body;
    let m = "";
    if (question.type === "multiple_choice_question")
        m = question.options.reduce(
            (text: string, element: string) => (text += "\n- " + element),
            ""
        );
    return firstSecondLine + m;
}
